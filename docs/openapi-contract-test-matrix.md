...
| Endpoint | Check |
|---|---|
| `POST /server/match-profile/build` | requires server headers, rejects unknown/revoked/expired server identities, rejects wrong realm/build, unsupported catalog returns `409 CATALOG_VERSION_NOT_SUPPORTED`, success returns match profile |
